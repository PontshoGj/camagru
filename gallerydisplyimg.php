<?php
        ...
        if (count($display))
        {
            while($i < count($display))
            {
                ...
                echo '<div style="float: left;"><form id="myForm" action="gallery.php" method="post"><input type="hidden" name="delete" value="'.$display[$i]['num'].'"><button type="submit" onclick="sub()" value="delete" name="submit">delete</button></form></div>';           
                $i++;
                $jj++;
                if ($jj == 5)
                    echo "<br/>";
